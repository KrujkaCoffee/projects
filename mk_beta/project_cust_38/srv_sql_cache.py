...
class FileRequestCache:
    ...
    def deserialize_payload(self, entry: dict):
        try:
            if entry is None or 'payload_bytes' not in entry:
                return None
            raw: bytes | memoryview | bytearray | None = entry['payload_bytes']
    ...
    def get_entry_meta(self, request_key: str) -> dict[str, Any] | None:
        meta = CPG.custom_request_pg(
            f'SELECT * FROM {REQUEST_CACHE_META_TABLE} WHERE request_key = {request_key!r}',
            one=True,
            rez_dict=True
        )
    ...
    def get_entry_payload(self, entry: dict):
        return self.serialize_payload(entry['payload_bytes'])
    ...
    def compute_policy(self, *, table_keys: Sequence[str],
    ...
    def store_entry(self, *, request_key: str, db_path: str, sql_text: str, params: Any, options: Mapping[str, Any],
        ...
        # CSQ.custom_request_c(repo.db_files, sql, list_of_lists_c=[[meta_record[col] for col in cols]])
        insert_cache_meta = CPG.custom_request_pg(sql, params=[[meta_record[col] for col in cols]])
        print(f'[store_entry] insert_cache_meta result: {insert_cache_meta}')
        ...
        print(f'[store_entry] insert_cache_tables result: {insert_cache_tables}')
        ...
        print('[store_entry] insert_fields_results results: ', insert_fields_results)
        entry = copy.deepcopy(meta_record)
    ...
    def delete_entry(self, request_key: str) -> None:
        ...
        print(f'[delete_entry] {state_delete_payload} {state_delete_payload_1} {state_delete_payload_2}')
    ...
    def invalidate_by_table_keys(self, table_keys: Sequence[str], *, notes: str = '') -> int:
        ...
        print(f'[invalidate_by_table_keys] result rows {rows}')
        ...
        print(f'[invalidate_by_table_keys] update inv rows {update_invalidated}')
