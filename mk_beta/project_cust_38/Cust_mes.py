...
import asyncio
import typing
from builtins import ValueError, tuple
...
class Pozitions(_Pozition_base):
    def __init__(self,p_noms:list,db_kpl,db_naryad,db_resxml,db_users,parent_self=None, load_loacal_graf=False,
        ...
        for i, item in enumerate(rows):
            ...
            self.dict_pozs[item['Пномер']] = Pozition(item,db_kpl,db_naryad,db_resxml,db_users,self.parent_self,
...
class Group_nar(_ImportDb):
    ...
    def split_by_group(set_nars_num:set[int],name:str, msg_jur:str, fragment:Fragment_jur)->bool: #27.04.2026
        ...
        def add_journals_for_remain_seconds(last_end_date: str, last_time_state: TimeState, nom_nar: int):
            remain = last_time_state.remain
            journal = Jurnal_nar(CFG.Config.project.db_naryad, user=fragment.user, nom_nar=nom_nar)
            while remain:
                start_state = calc_new_time(F.date_add_seconds(last_end_date, 1), is_start=True)
                start = start_state.date
                st_journal_id = journal.add_new_row(_dict_emploee_full, '', start, 'Начат',
                                                    msg_jur,
                                                    is_idle)
                if not st_journal_id:
                    return
                jur_obj.refresh()
                jur_obj.set_selected_fragment(st_journal_id)
                end_state = calc_new_time(F.date_add_seconds(start, remain), is_start=False)
                remain = end_state.remain
                end = end_state.date
                end_journal_id = jur_obj.add_new_row(_dict_emploee_full, '', end, end_status,
                                                     msg_jur,
                                                     is_idle)
                last_end_date = end
                if not end_journal_id:
                    return
        ...
        jur_obj.update_row(_dict_emploee_full, date_time=fragment.end_date, journal_id=fragment.end_s_num, lbl_abstract_text='', is_idle=main_nar_is_idle)
...
class Plan_custom_weekends():
    ...
    def _get_dict_weekends(self,blob_data='None')-> dict:
        ...
        if blob_data is None or blob_data == '':
            dict_weekends = dict()
        else:
    ...
    def upload(self):
        ...
        if self.id is None:
            result = CSQ.custom_request_c(CFG.Config.project.db_naryad,f"""INSERT INTO naryad_composit_files 
                              ({fields})
                              VALUES ({CSQ.questions_for_mask(data)}) RETURNING id""",list_of_lists_c=data,returning=True,rez_dict=True)
            self.id = result[0]['id']
        else:
...
class Compositions():
    ...
    def template(self)->list[dict]:
        return [_.template() for _ in self.comps]
...
class Composition_poz(_ImportDb):
    ...
    @property
    def is_coupled(self) -> bool:
        if self.count_aggregate == self.calc_summ_coupled() and self.count_aggregate >0:
            return True
        return False
    ...
    def load_source_table_for_join(self):
        if isinstance(self.source_table_for_join, int):
            ...
            raise Exception(f"place= {CFG.Config.place.Имя};  source_table_for_join not_found! field {
                self.source_table_for_join} for  tbl= {self.id} {self.name}")
...
class Checker_val_fields():
    ...
    def _check_date(self, val, field_o:Field_db_info):
        ...
        if not F.is_date(val, dateformat):
            self._dict_checked[name_field] = Checker_val_fields_rez(False,
                      f'"{alias}" Не корректный формат даты должна быть ГГГГ-ММ-ДД')
            return False
        self._dict_checked[name_field] = Checker_val_fields_rez(True)
        return True
...
class Fields_db_info():
    ...
    def __init__(self,CHECK_FIELD_RULES=None):
        ...
        rez = CSQ.custom_request_c(CFG.Config.project.db_kplan, f"""
            ...
            WHERE podrazdel.poki == {CFG.Config.place.poki} or podrazdel.poki IS NULL
            --{F.now()}""", rez_dict=True)  #
        if not rez:
        ...
        for it in rez:
            ...
            table.add_field(f)
            if table.id == 40 and CFG.Config.place.poki == 1:
                print(f'id field: {f.s_num} ({it})')
...
class Poz_gant():
        ...
        def calc_start_end(current_tbl,time_h)->tuple[str,str]:
            ...
            if start is None or start=='':
                if self.min_date_month:
                    ...
                    self.err_recalc.append(f'Не удается определить дату в КПЛ {self.poz_id} для {current_tbl.norm_full_name}.')
                    return '', ''
    ...
    def recalc_days(self,row_dates_etap_plan:dict,row_time_etap:dict,
                ...
                if tbl.is_composite:
                    for field in tbl.get_composite_fields(False):
                        if field.name_mes in row_time_etap:
                            tmp_dict_grps[gr].add_h(tbl.name, row_time_etap[field.name_mes],tbl.default_hours_day_gant)
                else:
                    if tbl.norm_full_name  in row_time_etap:
                        tmp_dict_grps[gr].add_h(tbl.name, row_time_etap[tbl.norm_full_name],tbl.default_hours_day_gant)


        tmp_day_dt = copy.copy(start_day )
        ...
        [_ for  _ in tmp_dict_grps.values() if _.is_filled]
        for gr, list_tbl_db in fields_db_info.tables_db.dict_groups_gant.items():
            gr_it = tmp_dict_grps[gr]
    ...
    def recalc(self,poz:Pozition,dict_template_cld: dict[datetime.datetime, Month_cld_day],
               ...
               recalc_days:datetime.datetime|None=None)->bool:
        self._clear()
...
class Gant():
    ...
    def recalc(self,ids:list|None,recalc_days:datetime.datetime|None=None)->bool:
    ...
    def oforml_table(self, app_self: mywindow, tbl, tbl_filtr: QtWidgets.QTableWidget = '',restore_selected_cell:bool=False) -> bool:
