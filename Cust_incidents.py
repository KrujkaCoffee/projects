...
"""
Независимый менеджер инцидентов MES.

Модуль намеренно импортирует только стандартную библиотеку. Его можно подключить
до PyQt и остальных модулей приложения: ранняя ошибка сначала попадёт в локальный
spool, а после запуска приложения будет перенесена в общее хранилище.

По умолчанию включён только сбор (MES_INCIDENT_MODE=collect). Создание задач Б24
включается явно режимом silent_test или production.

Основные переменные запуска:
    MES_INCIDENT_MODE=collect|silent_test|production|off
    MES_INCIDENT_ROOT=Z:\\MES_setup\\errors\\incidents
    MES_INCIDENT_TEST_USER_ID=<id тестового пользователя Б24>
    MES_INCIDENT_RESPONSIBLE_ID=<id ответственного в production>
    MES_INCIDENT_AUDITORS=<id,id,...>
    MES_INCIDENT_B24_URL=<webhook>; если не задан, URL лениво читается из Cust_b24

Безоконная проверка группировки и постановки тестовых задач:
    python -m project_cust_38.Cust_incidents --silent-probe
Команда работает только при MES_INCIDENT_MODE=silent_test.
"""
...
class IncidentConfig:
    ...
    def from_env(cls) -> IncidentConfig:
        shared = os.getenv("MES_INCIDENT_ROOT", r"Z:\MES_setup\errors\incidents")
        local = os.getenv(
            "MES_INCIDENT_LOCAL_ROOT",
            str(Path(tempfile.gettempdir()) / "mes_incidents"),
        )
        return cls(
            shared_root=Path(shared),
            local_root=Path(local),
            mode=IncidentMode.parse(os.getenv("MES_INCIDENT_MODE", "collect")),
            b24_url=os.getenv("MES_INCIDENT_B24_URL", "").strip(),
            b24_responsible_id=_env_int("MES_INCIDENT_RESPONSIBLE_ID"),
            b24_test_user_id=_env_int("MES_INCIDENT_TEST_USER_ID"),
            b24_created_by_id=_env_int("MES_INCIDENT_CREATED_BY_ID"),
            b24_auditors=_env_ids("MES_INCIDENT_AUDITORS"),
            allow_legacy_b24_url=_env_bool("MES_INCIDENT_ALLOW_LEGACY_B24", True),
            verify_tls=_env_bool("MES_INCIDENT_VERIFY_TLS", False),
            request_timeout_sec=_env_float("MES_INCIDENT_REQUEST_TIMEOUT", 8.0),
            b24_batch_size=max(1, _env_int("MES_INCIDENT_B24_BATCH") or 5),
            b24_retry_base_sec=max(
                0.0, _env_float("MES_INCIDENT_B24_RETRY_BASE", 15.0)
            ),
            flush_interval_sec=_env_float("MES_INCIDENT_FLUSH_INTERVAL", 20.0),
            lock_timeout_sec=_env_float("MES_INCIDENT_LOCK_TIMEOUT", 8.0),
            lock_stale_sec=_env_float("MES_INCIDENT_LOCK_STALE", 600.0),
            capture_qt_critical=_env_bool("MES_INCIDENT_CAPTURE_QT", True),
            capture_native_faults=_env_bool("MES_INCIDENT_CAPTURE_NATIVE", True),
            debug=_env_bool("MES_INCIDENT_DEBUG", False),
        )
...
class ExclusiveFileLock:
    """Простой межпроцессный lock. При timeout критическая секция не выполняется."""
...
class IncidentManager:
    ...
    def attach_safe_application(self, app: Any) -> bool:
        ...
        # CrashReporter сохранил ранний hook как предыдущий. Отключаем только
        # повторный захват; делегирование исходному hook продолжит работать.
        self._early_active = False
    ...
    def _spool_payload(self, payload: IncidentPayload) -> CaptureReceipt:
        ...
        try:
            ...
            # Последний резерв: пробуем сразу общее хранилище.
            try:
    ...
    def _resolve_b24_url(self) -> str:
        ...
        if not self.config.allow_legacy_b24_url:
            return ""
        # Импорт выполняется только в worker после инициализации приложения.
        # Частично импортированный Cust_b24 просто даст пустой URL и повтор позже.
        try:
    ...
    def flush_outbox(self, limit: int = 100) -> dict[str, int]:
        ...
        for path in paths:
            try:
                ...
                # Очередь уже обрабатывается другим процессом.
                continue
            except Exception as exc:
                result["errors"] += 1
                self._mark_outbox_error(path, str(exc), increment=True)
...
def run_silent_test_probe(
    ...
    """Создаёт два предсказуемых тестовых инцидента без запуска Qt/UI."""
