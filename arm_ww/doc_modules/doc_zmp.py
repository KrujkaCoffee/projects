...
def gen_data():
    refs, filtr_war = GEN.prepare_stor_vars('ЗаказМатериаловВПроизводствоТовары.Ссылка.Склад.Ссылка')
    text = f"""
   ВЫБРАТЬ
...
def fill_table(app_self,hook_prog_bar):
    tbl = DTCLS.app_self.ui.tbl_zmp
    tblf = DTCLS.app_self.ui.tbl_zmp_filtr
    hook_prog_bar.open()
    hook_prog_bar.set(0)
