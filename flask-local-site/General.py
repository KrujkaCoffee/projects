    ...
    def add_line(tbl,row):
        ...
        if poki == 0:
            ...
            if date_contract != '' and date_dirty != '' and F.is_date(date_dirty,"%Y-%m-%d"):
                otkl_dog = (F.strtodate(row['Упаковка'].split('/')[1],"%Y-%m-%d") - F.strtodate(date_contract,)).days
            ...
            tbl.append({
                'Направление': napr,  # 0
                         ...
                         'Поз.': poz_name,  # 4
                         'Кол-во': count,  # 5
                'Номенклатура ЕРП': row['Номенклатура_ЕРП'],                    # 29.06.2026 задача 100072745
                ...
                'Дата занесения в план месяца': date_add_plan_month,            # 29.06.2026 задача 100072745
                # 'Дата по ЗК': date_zk,                                        # 29.06.2026 задача 100072745
                    ...
                    'Даты обеспечения по этапам': str_dates_obesp,              # 29.06.2026 задача 100072745
            ...
            list_etaps_dates = [row['Раскрой'],
                                ...
                                row['Упаковка'],]
            poz_name = row['Позиция']
            comment_sbbsi = row['Примечание сббси']
            count = str(row['Количество'])
            if row['Кд'] != '' or row['Статус']=='Резерв':
                min_date = ''
            else:
            ...
            tbl.append({
                 'Направление': napr,  # 0
                 'Псевдоним': row['Псевдоним'],  # 1
                 'Номенклатура':row['Номенклатура_ЕРП'],   # 2
                 'Номер проекта': row['Номер проекта'],   # 3
                 'Номер заявки': py,   # 4
                 'Поз.': poz_name,   # 5
                 'Кол-во': count,   # 6
                 'Нормо-час сб': str(row['Нормо-час сб']),   # 7
                 'Статус': row['Статус'],   # 8
                 'Дата внесения в МЕС': date_add_kpl,   # 9 ?
                 'Требуемая дата КД': min_date,   # 10
                 'Ф.Дата получения КД': row['Кд'],   # 11
                 'Тех. МК': mk_is,   # 12
                 'Плановая дата начала раскр. участка': date_rezka_start,   # 13
                 'Текущая плановая дата зав. сборки': date_sb,   # 14
                 'Текущая плановая дата зав. упаковки': date_upak,   # 15
                 'Дата по договору': date_contract,   # 16
                 'Прогноз дата зав.сб.': prognoz_date,   # 17
                 'Откл. от дог.': otkl_dog,   # 18
                 'Примечание производства': row['Примечание_пл_оуп'],   # 18
                 'Примечание сб. участок': primech,
                ...
                'Примечание сббси. участок': comment_sbbsi,
...
"""
        ...
        custom_request_c = f"""SELECT 
                ...
                "`" AS "Сумм_н`Сумм_к",
                 ...
                 пл_сббси.Примечание_сббси as "Примечание сббси",
                 пл_оуп.Примечание as "Примечание_пл_оуп",
                ...
                LEFT JOIN пл_осил ON пл_осил.НомПл = plan.Пномер 
                LEFT JOIN пл_оуп ON пл_оуп.НомПл = plan.Пномер 
                LEFT JOIN пл_сббси ON пл_сббси.НомПл = plan.Пномер 
