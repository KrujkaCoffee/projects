...
def btns(self: mywindow):
    ...
    ui.btn_del_dates_etaps.clicked.connect(lambda: GKPL.del_dates_etaps(self))
...
def tbls(self: mywindow):
    ...
    ui.tbl_preview.verticalHeader().sectionClicked.connect(lambda ind: GKPL.tbl_preview_on_vert_header_click(self, ind))
    ui.tbl_preview.doubleClicked.connect(lambda: KPL.show_fact_etap_by_current_day(self))
    ui.tbl_preview.verticalScrollBar().valueChanged.connect(
        ui.tbl_limit_gant.verticalScrollBar().setValue)
