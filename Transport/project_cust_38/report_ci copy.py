...
def vibor_sort_c_report_c(self:mywindow, *args):
    ...
    if vid == 'Исполнение плана месяца':
        ...
        list_month = CSQ.custom_request_c(self.db_kplan,f"""SELECT Дата, Пномер FROM mnts_plan WHERE  file_poz_plan NOT NULL;""",rez_dict=True)
    ...
    if vid == 'Трудозатраты':
        ...
        self.ui.le_end_of_period.setText(konec)
        self.ui.le_start_of_period.setText(nach)
        list_podr = sorted(list(set([_['Подразделение'] for _ in self.Data.ETAP_BY_FIO.values() if _['этап'] != None])))
        self.ui.cmb_podrazdelenie.setDisabled(False)
        self.ui.cmb_podrazdelenie.clear()
        self.ui.cmb_podrazdelenie.addItem('')
...
@CQT.onerror
def report_c(self:mywindow, *args):
    ...
    clear_graf(self)
    if vid == 'Исполнение плана месяца':
        rez_spis = ispoln_pl_month(self.db_kplan,self.db_resxml,self.bd_naryad,self.Data.DICT_PROFESSIONS,self.Data.DICT_VID_RABOT,podrazd)
    if vid == 'Распределение работ по направлениям':
...
def gr_ud_proizv_cexa(self:mywindow, nach, konec, *args):
    def get_ves_from_res_and_xml():
        ...
        for item in rez_mk:
            ...
            CMS.calc_and_fill_weight_by_xml_and_res(self, self.db_resxml, self.bd_naryad, self.bd_mat, nom_mk, kol_vo_izd, DICT_FILTR,
                                                DICT_MAT)
            #try:
    ...
    custom_request_c = f"""SELECT plan.МК, пл_ко.Вес_ВО, "" as used FROM plan INNER JOIN пл_ко ON пл_ко.НомПл = plan.Пномер WHERE plan.МК != 0"""
    ...
    for item in rez_mk:
        ...
        print(f"Номер_заказа {item['Номер_заказа']}   Номер_заказа {item['Номер_заказа']}   Пномер {item['Пномер']}  Вес: {item['Вес']} xml: {F.valm(item['xml'])}")
    ...
    for item in data:
        if F.is_date(item['Дата_создания']):
            ...
            if not item['Код_вп'] == 'возражаю(см_пояснение)':
                dict_month[month][kod_vp+ "_принятые"] += 1
    ...
    if self.vid_report_c ==  'Журнал замечаний динамика':
        ...
        for podr in args[0][0].keys():
            if podr != 'month':
                ...
                podr_kod = podr
                if "_" in podr:
                    podr_kod = podr.split("_")[0]
                if podr_kod in self.DICT_RC:
                    color = f'rgb({self.DICT_RC[podr_kod]["Цвет"]})'
                colors.append(color)
        ...
        labels = []
        for kod in labels_cod:
            for rc in self.DICT_RC.keys():
                kod = kod.replace(rc,self.DICT_RC[rc]['Сокр_наим_СТО'])
            labels.append(kod)
...
def calendar_select(self, *args):
    ...
    start, end = F.start_end_dates_c(data,"%Y-%m-%d %H:%M:%S",'m', "%Y-%m-%d")
...
def ready_procent(self):
    ...
    headers = ['Номер проекта','План покраска(ч)', 'План Лазерная резка(ч)', 'План мех обработка(ч)', 'План сборочно-сварочных работ(ч)', 'Фактически отработанное время по МК(ч)', 'общий % выполнения сборочно-сварочных работ', 'Плановое время сборочно-сварочных работ за месяц(ч)', 'работы выполненные в текущем месяце']  #, 'Выполнено в текущем месяце'] # проект, время итого на сборку сварку, потреблено времени Х, осталось времени Y
    res.append(headers)
...
def analysis_vneplan_by_vid_rab(self:mywindow, nach, konec, podrazd=None, *args):
    ...
    DICT_KAT_VNEPL = F.deploy_dict_c(CSQ.custom_request_c(self.bd_naryad, f"""SELECT * FROM kategor_vnepl WHERE kod > 0""",
                                          rez_dict=True), 'value')
    RESP_PROF = CSQ.custom_request_c(self.bd_users,
                ...
                ON vid_rab_po_dolg.Вид_работ = professions.вид_работ,
                 group_vid_rab_for_plan ON group_vid_rab_for_plan.name=vid_rab_po_dolg.group_for_plan'''
    ...
    for type_nar, etap, kat, time, item in list_vid_rab:
        if kat in DICT_KAT_VNEPL:
            if DICT_KAT_VNEPL[kat]['Коэффициент_наряда'] != 1:
                continue
        jur_obj = CMS.Jurnal_nar(self.bd_naryad,item['Пномер'])
        ...
        tmp_dict = {'Дата': item['Дата_журнал'],
                        'Этап' : etap,
    ...
    if CQT.msgboxgYN(f'Обновить процент внеплановых работ в таблице видов по направлениям?'):
        ...
        for item in res_list:
            if item['Вид_по_напр'] not in dict_vids_napr_percent:
                dict_vids_napr_percent[item['Вид_по_напр']] = {'p':0,
                                                               'v':0}
            if item['Тип'] == 'внеплан':
                dict_vids_napr_percent[item['Вид_по_напр']]['v']+=item['time']
            else:
                dict_vids_napr_percent[item['Вид_по_напр']]['p'] += item['time']
        ...
        for vid in dict_vids_napr_percent.keys():
            ...
            CSQ.custom_request_c(self.db_kplan,f"""UPDATE виды_по_напр SET (vneplan_percent) = {delta} WHERE Пномер = {vid}""")
    ...
    for mk in set_mk:
        ...
        tmp_dict = {'Дата': mk.Дата_завершения,
                    'Этап': '',
                    'Направление': mk.Вид,
                    'ПУ': mk.Номер_заказа,
...
def analysis_effectiv_work_per_minute(self:mywindow, nach, konec, podrazd=None, *args):
    ...
    for item in list_mk:
        ...
        if mk.Тип_мк_Имя == 'Плановая':
            ...
            dict_moth[nom_kpl]['План_рес_ед(мин.)'] = summ_dict_vid_rab(dict_moth[nom_kpl]['План_рес_ед(мин.)'], mk.get_dict_vid_rab(DICT_PROF_ALL))
            dict_moth[nom_kpl]['План_освоено_ед(мин.)'] = summ_dict_vid_rab(dict_moth[nom_kpl]['План_освоено_ед(мин.)'], mk.get_dict_vid_rab(DICT_PROF_ALL,True))
            dict_moth[nom_kpl]['month'] = month
        else:
            dict_moth[nom_kpl]['Внеплан_кол'] = summ_dict_vid_rab(dict_moth[nom_kpl]['Внеплан_кол'], mk.get_dict_vid_rab(DICT_PROF_ALL,True,by_one_izd=False))
    ...
    for num_kpl in dict_moth.keys():
        ...
        dict_moth_calc_ed[month]['План_рес_ед(мин.)'] = summ_dict_vid_rab(dict_moth_calc_ed[month]['План_рес_ед(мин.)'], dict_moth[num_kpl]['План_рес_ед(мин.)'])
        dict_moth_calc_ed[month]['План_освоено_ед(мин.)'] =  summ_dict_vid_rab(dict_moth_calc_ed[month]['План_освоено_ед(мин.)'], dict_moth[num_kpl]['План_освоено_ед(мин.)'])
        dict_moth_calc_ed[month]['Внеплан_ед(мин.)'] = summ_dict_vid_rab(dict_moth_calc_ed[month]['Внеплан_ед(мин.)'], dict_moth[num_kpl]['Внеплан_кол'],dict_moth[num_kpl]['Кол_во_план'])
    
    for month in dict_moth_calc_ed.keys():
        dict_moth_calc_ed[month]['Отклонение_в_%'] = calc_deviation_dicts_vid_rab(dict_moth_calc_ed[month]['План_рес_ед(мин.)'], dict_moth_calc_ed[month]['Внеплан_ед(мин.)'])
        dict_moth_calc_ed[month]['План_рес_ед(мин.)'] = {k:round(v) for k,v in dict_moth_calc_ed[month]['План_рес_ед(мин.)'].items()}  
        dict_moth_calc_ed[month]['План_освоено_ед(мин.)'] ={k:round(v) for k,v in dict_moth_calc_ed[month]['План_освоено_ед(мин.)'].items()}  
        dict_moth_calc_ed[month]['Внеплан_ед(мин.)'] ={k:round(v) for k,v in dict_moth_calc_ed[month]['Внеплан_ед(мин.)'].items()} 
...
, naryad.Опер_время 
 FROM naryad INNER JOIN mk ON mk.Пномер = naryad.Номер_мк,
  Тип_мк ON Тип_мк.Пномер = mk.Тип WHERE mk.Статус == "Открыта" '''
    ...
    res = [['Номер_МК','Номер_заказа','Номер_проекта','Вид','Номенклатура','Тип','Вес,кг*(с поправкой)','Всего, н-см.','Освоено, н-см.',"Процент %",'Вес_осталось,кг']]
    for nom_mk in dict_mk:
        plan = 0
        fact = 0
...
def jurnal_rabot(self, data_nach, data_kon, *args):
    custom_request_c = f"""SELECT mk.Номер_проекта, mk.Номер_заказа, jurnal.Дата, jurnal.ФИО AS "ФИО_журнал", jurnal.ФИО AS "Должность", jurnal.Статус, 
            jurnal.Подытог, jurnal.Подытог, jurnal.Подытог_нормы as "Для трудозатрат", jurnal.Примечание AS "Примеч_журнал", 
...
def virabotka_sotrudnikov(self, data_nach, data_kon, etap, *args):
    if F.strtodate(data_nach) >= F.strtodate('2024-07-01 00:00:00'):
        ...
        if 'shift' in CQT.get_key_modifiers(self):
            CALC_BASE_ONLY_PREM = True
    spis_vir_sotr, err, DICT_MASTERS = CMS.calc_productivity_c(data_nach, F.bdcfg('BD_users'), F.bdcfg('Naryad'), F.bdcfg('BDact'),
                                         self.DICT_EMPLOEE_FULL,self.DICT_PRICE_BRAK,CALC_BASE_ONLY_PREM=CALC_BASE_ONLY_PREM)
    ...
    rez = [
        ['Месяц', 'ФИО', 'Профес.', 'Выработка включая вычет,%','Выработка чистая,%', "Вычет,%","КТУ%",
         "Закрыто, час. с коэфф.","Закрыто, час. без коэфф.", "Ставка, час.", "Список нарядов",'Режим','Подытог норм, час.']
    ]
...
jurnal.Статус = "Завершен" AND jurnal.ФИО = "{CMS.name_by_empl_c(empl)}") AND 
 datetime(jurnal.Дата) <= datetime("{data_kon}") AND 
datetime(jurnal.Дата) >= datetime("{data_nach}") AND jurnal.ФИО == "{CMS.name_by_empl_c(empl)}"
"""
    ...
    dict_user_brak = get_summ_brak_fio(self.DICT_PRICE_BRAK, CMS.name_by_empl_c(empl), get_jur_brak(self.bd_naryad,data_nach, data_kon))
    ...
    for i, item in enumerate(rez_jur):
        if rez_jur[i]['ФИО2'] == CMS.name_by_empl_c(empl):
    ...
    if miutes == 0:
        ...
        if CALC_BASE_ONLY_PREM:
            prem = round((summ_tvr_koef ) / miutes * 100)
            prem_max = round((summ_tvr_max ) / miutes * 100)
        else:
            prem = round((summ_tvr_koef ) / miutes * 200)-100
            prem_max = round((summ_tvr_max ) / miutes * 200)-100
    rez_jur[-1]['Дата'] = f'Эффективность {eff}%'
    rez_jur[-1]['Примеч_журнал'] = f'По табелю {round(miutes / 60, 1)} час.'
    brak = round(dict_user_brak['Сумма'], 1)
    prem -= brak
    if prem < 0:
        prem = 0
    ...
    rez_jur[-1]['Твремя'] = f'{round((summ_tvr) / 60, 1)} час.'
    rez_jur[-1]["Подытог Норм"] = f'{round((summ_poditog) / 60, 1)} час.'
    rez_jur[-1]['Коэфф_сложности'] = f'Вычет БРАК {brak}%'
    rez_jur[-1]['Коэфф_вых'] = f'Общ. итог {round((summ_tvr_koef ) / 60, 1)} час.'
...
def virabotka_ceha_ponaryadno(self, data_nach, data_kon, etap, *args):
    ...
    ponaryadno = [['Проект', 'Вeс_мк', 'Номер_наряда', 'Твремя_мин', f'кг_наряд({PROIZVODITELNOST_POST_SM} пр-ть)', 'Фвремя_мин', "Примечание", "Категория_внепл", "Вид",'Направление']]
    for oper in list_oper:
...
def plan_fact_grafic_mes(self, data_nach, data_kon, *args):
    ...
    table_vnepan = F.list_of_lists_to_dict_of_dicts(vneplan_po_napravl(self,data_nach, data_kon,'Все',generate_graf= False),'Месяц')
    ...
    list_kat_vnepl = CSQ.custom_request_c(self.bd_naryad, f"""SELECT value FROM kategor_vnepl WHERE kod > 0""", one_column=True,
                                hat_c=False)
    rez = [['Месяц',  'Факт, уд.т.', 'План(ПДО), н-см.', 'Освоено плановых, н-см.', 'Внеплан, %',
            'Освоено(план+внеплан работы), н-нед.']]
    for item in list_month_plan:
        ...
        plan_normo_nedel = round((item['Нормо_смены_сб'] + item['Нормо_смены_св'] + item['Нормо_смены_зачист']),2)
     #rez = [['Месяц',    'План, т.',              'Отгрузка, т.', '           Факт, н-нед.'
        tmp = [data,  round(summ_ves/1000 , 2), plan_normo_nedel, round(summ_vrem_osvoeno * KOEF_VNEPLANA,2),
               summ_ves_vir_vneplan_proc, round(summ_ves_fact*KOEF_VNEPLANA,2)]
...
"""
    ...
    for item in responce:
        ...
        if not check_norm(item,tmp_dict):
            continue
        tmp_dict['Время_на_проект_факт_дн'] = (F.strtodate(tmp_dict['Дата_отгрузки']) - F.strtodate(tmp_dict['Дата_ЗП'])).days
...
def udel_trud_sort_c(self:mywindow, data_nach, data_kon, *args):
    shablon_etaps = dict()
    rez = [['Код из бд','Выборка,шт.',"Направление",'Наименование (мин/кг)','кг_на_пост_см_факт','кг_на_пост_см_норм','кг_на_пост_см_средн','кг_пост_см_кпл']]
    ...
    for item in responce:
        if item['Вид'] not in dict_kpl:
            dict_kpl[item['Вид']] = {'Нчпс':0,'Вес':0, 'Кол_во':0, 'Нчас_заг':0,'Нчас_сб':0,'Нчас_св':0,'Нчас_зач':0, 'Нчас_покр':0, 'Нчас_мехобр':0, 'Нчас_вспом':0}
        ...
        npsm = (F.valm(item['Вес_ВО'])/kol)/(((item['Нчас_сб']+item['Нчас_св']+ 0.5*item['Нчас_зач'])/kol)/16)
        if npsm < 50 or npsm > 300:
            continue
        ...
        if item['Номер_мк'] in dict_mk_sort_c:
            ...
            if item['Номер_мк'] not in tmp_dict_mk:
                tmp_dict_mk[item['Номер_мк']] = {'Вес':item['Вес'],
                                                 'Вид': dict_mk_sort_c[item['Номер_мк']],
    ...
    for mk in tmp_dict_mk.keys():
        ...
        for etap in tmp_dict_mk[mk]['Этапы'].keys():
            if etap == 'Сборка+сварка':
                dict_shabl_napr[vid]['etap_fakt'][etap] += tmp_dict_mk[mk]['Этапы'][etap] * tmp_dict_mk[mk]['koef_pogr_norm'] * tmp_dict_mk[mk]['koef_vneplana']
                ...
                dict_shabl_napr[vid]['etap_fakt'][etap] += tmp_dict_mk[mk]['Этапы'][etap] * tmp_dict_mk[mk]['koef_pogr_norm']
                dict_shabl_napr[vid]['etap_norm'][etap] += tmp_dict_mk[mk]['Этапы_нома'][etap] * tmp_dict_mk[mk][
                    'koef_pogr_norm']
    for vid in dict_shabl_napr.keys():
        ...
        if ves == 0:
            continue
        tmp = [vid, kol_vo, self.Data.NAPRAVL_DEYAT_KOD[dict_sort_c[vid]['Направл']]['Направление'], dict_sort_c[vid]['Имя'], 0, 0, 0, 0 ]
        ...
        summ_sb_sv = dict_shabl_napr[vid]['etap_fakt']['Сборка+сварка'] + dict_shabl_napr[vid]['etap_fakt']['Зачистка']*0.5
        summ_sb_sv_norm = dict_shabl_napr[vid]['etap_norm']['Сборка+сварка'] + dict_shabl_napr[vid]['etap_norm']['Зачистка'] * 0.5
        if summ_sb_sv:
            tmp[4] = 0
            tmp[5] = 0
            tmp[6] = 0
        ...
        for rez_etap in rez[0][8:]:
            ...
            if dict_shabl_napr[vid]['etap_fakt'][rez_etap] != 0 and dict_shabl_napr[vid]['etap_norm'][rez_etap] != 0:
                tmp.append(round((dict_shabl_napr[vid]['etap_fakt'][rez_etap] + dict_shabl_napr[vid]['etap_norm'][rez_etap])/2,3))
        rez.append(tmp)
    #rez.append(['' for _ in rez[0]])
    #rez.append(['' for _ in rez[0]])
    return rez
...
def get_plan_vneplan_data(self, data_nach, data_kon, vid='Все', etap='Сборка+сварка'):
    ...
    list_kat_vnepl = CSQ.custom_request_c(self.bd_naryad, f"""SELECT value FROM kategor_vnepl WHERE kod > 0""",
                                          one_column=True,
                                          hat_c=False)
    dict_type_mk_names = F.deploy_dict_c(CSQ.custom_request_c(self.bd_naryad, f"""SELECT * FROM Тип_мк""", rez_dict=True),
                                         'Имя')
    dict_type_dorez = CSQ.custom_request_c(self.bd_naryad, f"""SELECT * FROM тип_дорезок""", rez_dict=True)
    dict_type_dorez_names = F.deploy_dict_c(dict_type_dorez, 'Имя')
    dict_type_dorez_nums = F.deploy_dict_c(dict_type_dorez, 'Пномер')

    dict_type_dorab_ = CSQ.custom_request_c(self.bd_naryad, f"""SELECT * FROM тип_доработок""", rez_dict=True)
    dict_type_dorab_names = F.deploy_dict_c(dict_type_dorab_
                                            , 'Имя')
    dict_type_dorab_nums = F.deploy_dict_c(dict_type_dorab_
                                           , 'Пномер')
    dict_dorezok = F.deploy_dict_c(CSQ.custom_request_c(self.bd_naryad, f"""SELECT дорезки_мк.Номер_мк,  тип_дорезок.Имя 
         FROM дорезки_мк INNER JOIN тип_дорезок ON тип_дорезок.Пномер == дорезки_мк.Причина""", rez_dict=True), "Номер_мк")
    ...
    for item in list_month_plan:
        ...
        DICT_VID_NAPR = F.deploy_dict_c(CSQ.custom_request_c(self.db_kplan,f"""SELECT НомПл, Вид  FROM пл_топ;""",rez_dict=True),'НомПл')
        ...
        custom_request_c = f"""SELECT DISTINCT
                        ...
                        mk.Вес, mk.Номер_заказа, mk.Номер_проекта, mk.Дата_завершения, mk.Количество, 
                        mk.Номенклатура, mk.НомКплан, jurnal.Дата as Дата_журнал , jurnal.ФИО as fio_jur_zav, 
                        kategor_vnepl.value as Категория_внепл , 
                        naryad.Подтвержд_вып_дата as Подтвержд_вып_дата FROM jurnal
                                    INNER JOIN naryad ON jurnal.Номер_наряда = naryad.Пномер  
                                    INNER JOIN mk ON mk.Пномер = naryad.Номер_мк  
                                    INNER JOIN kategor_vnepl ON kategor_vnepl.kod = naryad.Категория_внепл   
                                    INNER JOIN Тип_мк ON Тип_мк.Пномер = mk.Тип 
                                    INNER JOIN тип_доработок ON тип_доработок.Пномер = mk.Тип_доработки  
                        WHERE {postfix} naryad.Внеплан != 1 AND naryad.Подтвержд_вып == 1 and 
                        jurnal.Статус == "Завершен" and datetime(jurnal.Дата) >= datetime("{nach_data}") 
                        and datetime(jurnal.Дата) <= datetime("{kon_data}")
            """
                ...
                if item_zav_nar['Тип'] == 'Плановая':  # Плановая
                    if item_zav_nar['Внеплан'] == 0:
                        ...
                        if item_zav_nar['Категория_внепл'] in dict_cat_vnepl:
                            ...
                            list_by_vid_rab.append(['внеплан',etap_user, item_zav_nar['Категория_внепл'], add_time, item_zav_nar])
...
def vneplan_po_napravl(self, data_nach, data_kon, vid, etap='Сборка+сварка', generate_graf = True, *args):
    rez, *args = get_plan_vneplan_data(self, data_nach, data_kon, vid, etap)
    if rez == None:
        return 
...
def trudozatraty(self, data_nach, data_kon, podrazd='-', *args):
    ...
    def check_master(self,dolg, fio, *args):
        ...
        for rc in self.DICT_RC_FULL.keys():
            if self.DICT_RC_FULL[rc]['ФИО_1см'] == fiod or self.DICT_RC_FULL[rc]['ФИО_2см']  == fiod or self.DICT_RC_FULL[rc]['ФИО_3см']  == fiod:
    ...
    FROM (SELECT jurnal.ФИО, jurnal.Подытог, mk.Пномер, mk.Номенклатура, mk.Номер_заказа, mk.Номер_проекта 
     FROM jurnal INNER JOIN naryad ON jurnal.Номер_наряда = naryad.Пномер INNER JOIN mk ON mk.Пномер = naryad.Номер_мк 
    WHERE jurnal.Подытог <> 0 AND jurnal.Статус = 'Начат' and naryad.Внеплан == 0  
    ...
    for i in range(len(rez_jur)):
        ...
        if fio in self.DICT_EMPLOEE_FULL_WITH_DEL:
            if self.Data.ETAP_BY_FIO[fio]['Подразделение'] == podrazd:
                set_mk.add("|".join([str(rez_jur[i]["Пномер"]),rez_jur[i]["Номенклатура"],rez_jur[i]["Номер_заказа"],rez_jur[i]["Номер_проекта"]]))
                rez.append({'ФИО':fio,
...
@CQT.onerror
def ispoln_pl_month(db_kplan,db_resxml,bd_naryad, DICT_PROFESSIONS, DICT_VID_RABOT, month_str:str):
    def calc_count_vipusk_erp(name_poz,num_py,zp_date):
    ...
    list_pnums =  [ _ for _ in plan.data.keys() if 'Тип' not in plan.data[_] or ('Тип' in plan.data[_] and plan.data[_]['Тип'] != 'Внеплан') ]
    ...
    req = f"""SELECT plan.Пномер,  "План" as Статус, napravl_deyat.Псевдоним as "Направление", пл_оуп.Дата_заявки_на_произв, пл_оуп.№ERP, пл_оуп.№проекта, plan.Позиция, 
        ...
        status_poz.Имя as "Статус позиции", "" as "Примечание", "" as "Всего н-смен на поз.","" as "Зав_мк из" FROM plan 
    ...
    list_all_nar_by_month = CSQ.custom_request_c(bd_naryad,f"""SELECT * FROM naryad WHERE 
        date(Подтвержд_вып_дата) >= datetime("{data_nach}") and date(Подтвержд_вып_дата) < datetime("{data_kon}")""",rez_dict=True)
    ...
    for i in range(len(rez)):
                                        ...
                                        for oper in dse_res['Операции']:
                                            if oper['Опер_номер'] == dse['Операции_номер']:
                                                ...
                                                if prof in DICT_PROFESSIONS:
                                                    ...
                                                    dict_vip_rab[nick] += dse['Норма_времени_пооперационно'] #nar_obj.Твремя * nar_obj.count_users()
                                                break
                                        break
                            else:
        ...
        for vid_r in list_vid_rab:
            if rez[i]['Пномер'] not in plan.data:
                ...
                ostatok_n_sm = 0
                if rez[i]['Пномер'] in plan.data and plan.data[rez[i]['Пномер']]['Группы_работ'][vid_r]['Норма_н_см'] > 0:
        ...
        count_vipusk_erp = calc_count_vipusk_erp(rez[i]['Номенклатура_ЕРП'],rez[i]['№ERP'],rez[i]['Дата_заявки_на_произв'])
