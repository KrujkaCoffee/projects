...
def apply_alias_list(list_resp, dict_alias):
    if list_resp == []:
        return list_resp
    if isinstance(list_resp[0],dict):
        for i in range(len(list_resp)):
            for key in list_resp[i]:
                if key in dict_alias:
                    list_resp[i][dict_alias[key]] = list_resp[i].pop(key)
    else:
        for j in range(len(list_resp[0])):
            for k in dict_alias.keys():
                if k == list_resp[0][j]:
                    list_resp[0][j] = dict_alias[k]
                    break
    return list_resp
