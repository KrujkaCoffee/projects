import flet as ft
import components.calc_airslide_back as calc_airslide_back
...
def apply_page_settings(page: ft.Page,DICT_MODULES_ROUTES:dict):
    ...
    Data.Data_module.cust_data: calc_airslide_back.Cust_module_params = calc_airslide_back.Cust_module_params()
def gen_page(page):
    Data: DTCLS.Data_page = page.data

    def new_calc(e: ft.ControlEvent, default_vals_input: dict | None = None):
        Data: DTCLS.Data_page = e.page.data
        Data.Data_module.cust_data: calc_airslide_back.Cust_module_params
        ...
        input_table_datatable, table_data = calc_airslide_back.generate_input_data(fnc_onchange_tbl_input,
                                                                                 _input_tabe_ref,
                                                                                 default_vals_input)
    ...
    def generate_desktop_row(page: ft.Page, rail: ft.NavigationRail):
        ...
        def add_rez_table(e: ft.ControlEvent):
            Data.Data_module.cust_data: calc_airslide_back.Cust_module_params
            page = e.page
            tbl_rez, table_data, btn_enabled = calc_airslide_back.generate_rez_tbl(e, _input_tabe_ref.current,
                                                                                 _output_tabe_ref)
            if tbl_rez == None:
                return
            ...
            _header_input_panel_textfield_ref.current.value = calc_airslide_back.get_name_new_calc(Data.Data_module.alias)

            page.update()

        def grab_new_table(e: ft.ControlEvent):
            dict_vals = calc_airslide_back.get_vals_from_input_data_tbl(_input_tabe_ref.current)
        ...
        def click_save_rez(e):
            ...
            rezult_data_for_save = calc_airslide_back.generate_rezult_data_for_save(name, _input_tabe_ref.current,
                                                                                  _output_tabe_ref.current)
            rez = calc_airslide_back.save_exel(rezult_data_for_save['input'], rezult_data_for_save['output'],
                                             rezult_data_for_save['name'], dir_save=cfg_module.sub_dir)
            if not rez:
                show_msgbox_err(e)
                return
            else:
                if not calc_airslide_back.save_in_db(e, name):
                    return
    ...
    def select_destination(e: ft.ControlEvent):
        def load_history_dest(e: ft.ControlEvent):
            def selectedRowsfnc(e: ft.ControlEvent, row_index: int, row_data: CMF.Row_data):
                ...
                input_tbl, output_tbl = calc_airslide_back.load_from_db_history_calc(e.page.data, s_num)
