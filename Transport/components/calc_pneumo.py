import flet as ft
import components.calc_pneumo_back as calc_pneumo_back
...
NAME_MODULE = "pneumatic_transport"
...
def gen_page(page):
    ...
    def new_calc(e: ft.ControlEvent, default_vals_input: dict | None = None):
        ...
        Data.Data_module.cust_data: calc_pneumo_back.Cust_module_params
        ...
        input_table_datatable, table_data = calc_pneumo_back.generate_input_data(fnc_onchange_tbl_input,
                                                                                 _input_tabe_ref,
                                                                                 default_vals_input)
    ...
    def generate_desktop_row(page: ft.Page, rail: ft.NavigationRail):
        ...
        def add_rez_table(e: ft.ControlEvent):
            Data.Data_module.cust_data: calc_pneumo_back.Cust_module_params
            page = e.page
            tbl_rez, table_data, btn_enabled = calc_pneumo_back.generate_rez_tbl(e, _input_tabe_ref.current,
                                                                                 _output_tabe_ref)
            if tbl_rez == None:
                return
            ...
            _header_input_panel_textfield_ref.current.value = calc_pneumo_back.get_name_new_calc(Data.Data_module.alias)

            page.update()

        def grab_new_table(e: ft.ControlEvent):
            dict_vals = calc_pneumo_back.get_vals_from_input_data_tbl(_input_tabe_ref.current)
        ...
        def click_save_rez(e):
            ...
            if not rez:
                ...
                if not calc_pneumo_back.save_in_db(e, name):
                    return
    ...
    def select_destination(e: ft.ControlEvent):
        def load_history_dest(e: ft.ControlEvent):
            def selectedRowsfnc(e: ft.ControlEvent, row_index: int, row_data: CMF.Row_data):
                ...
                input_tbl, output_tbl = calc_pneumo_back.load_from_db_history_calc(e.page.data, s_num)
            ...
            tbl_data = calc_pneumo_back.make_history_tbl_data(e.page.data)
