# -*- coding: utf-8 -*-
"""PostgreSQL runtime и безопасный Stage 2 probe для MES.

Модуль решает две разные задачи:

1. ``PostgreSqlExecutor`` — будущий прямой исполнитель PG-native SQL с
   process-local пулом, привычными режимами результата ``custom_request_c`` и
   явными многошаговыми транзакциями.
2. ``Stage2Probe`` — неблокирующая теневая проверка ``SELECT 123``. Основной
   поток приложения только кладёт событие в ограниченную очередь. Сетевые
   ошибки, таймауты и переполнение очереди никогда не меняют результат
   рабочего SQLite-запроса.

SQL SQLite -> PostgreSQL здесь намеренно не преобразуется. На этапе cutover
можно передать проверенный ``sql_preparer`` в ``PostgreSqlExecutor``.

Зависимости ``psycopg``/``psycopg_pool`` импортируются лениво: само добавление
модуля не ломает клиентов, где PG-драйвер ещё не установлен.
"""
...
class PostgresRuntimeError(RuntimeError):
    """Базовая ошибка нового PostgreSQL runtime."""
...
class ExecutorConfig:
    ...
    Для desktop-клиента безопасный профиль — ``min_size=0`` и
    ``max_size=1``: соединение создаётся лениво, переиспользуется в активный
    период и удаляется после ``max_idle_sec``.
    """
...
@dataclasses.dataclass(frozen=True, slots=True)
class ProbeConfig:
    """Параметры неблокирующего Stage 2 эксперимента."""
    ...
    def from_env(cls, prefix: str = "MES_PG_PROBE_") -> "ProbeConfig":
        ...
        return cls(
            ...
            telemetry_table=env.get(prefix + "TELEMETRY_TABLE") or None,
...
def _normalize_db_alias(value: Any) -> str:
    ...
    if "SRV:" in text:
        ...
        # Исторический формат допускает хвост после alias через slash/backslash.
...
def _first_statement_keyword(sql: str) -> str:
    ...
    ``WITH`` намеренно остаётся ``WITH``: CTE может содержать изменение данных,
    поэтому такой запрос не считается безопасным для автоповтора.
    """
...
def _native_sql_preparer(sql: str, params: Any) -> PreparedQuery:
    ...
    if isinstance(first, Sequence) and not isinstance(first, (str, bytes, bytearray, memoryview)):
        ...
        if _is_obviously_read_only(statement_type):
            # Это историческая семантика custom_request_c для SELECT: используется первая строка.
            return PreparedQuery(sql=sql, params=rows[0], many=False, statement_type=statement_type)
        return PreparedQuery(sql=sql, params=rows, many=True, statement_type=statement_type)
...
class _ProcessLocalPool:
    ...
    def _after_fork_child(self) -> None:
        # В дочернем процессе нельзя трогать worker threads и сокеты родителя.
        self._lock = threading.RLock()
        self._pool = None
        self._pid = os.getpid()
        self._generation += 1
    ...
    def _configure_connection(self, conn: Any) -> None:
        with conn.cursor() as cursor:
            # Один round trip вместо трёх на cold connection.
            cursor.execute(
                """
                SELECT
    ...
    def _new_pool(self) -> Any:
        ...
        pool = psycopg_pool.ConnectionPool(
            ...
            kwargs={
                ...
                # libpq округляет connect_timeout до целых секунд.
...
class PostgreSqlExecutor:
    ...
    def custom_request_c(
        ...
        ``lazy_method_hours`` принят ради совместимости, но кэширование не
        выполняется: до cutover за него отвечает существующий фасад. ``debug``
        также не печатает SQL, чтобы не раскрывать значения/PII.
        """
    ...
    def _execute_on_cursor(
        ...
        *,
        ...
        columns: list[str] = []
        if prepared.many:
            # Psycopg 3 требует returning=True, чтобы сохранить RETURNING при executemany.
    ...
    def probe_once(self, *, timeout_sec: float) -> float:
        ...
        with self._pool.connection(timeout_sec=timeout_sec) as conn:
            # Соединение работает в autocommit; session statement_timeout
            # задаётся один раз в configure callback, а не на каждый probe.
    ...
    def upsert_probe_snapshot(
        ...
        *,
        ...
        # UPSERT абсолютного snapshot идемпотентен: повтор не удваивает счётчики.
        with self._pool.connection(timeout_sec=timeout_sec) as conn:
...
class Stage2Probe:
    """Fire-and-forget probe: ни одна PG-ошибка не выходит в GUI/SQLite путь."""
    ...
    def observe(self, bd: Any, sql: str) -> bool:
        ...
        try:
            ...
            # Даже дефект telemetry не должен менять результат пользовательского запроса.
            try:
        ...
        while not self._stop_event.is_set() or not self._queue.empty():
            try:
                ...
                if (
                    ...
                    # Финальный snapshot выполняется один раз. После него
                    # telemetry больше не трогает pool, и max_idle может
                    # действительно закрыть backend-соединение.
...
def custom_request_c(
    ...
    """Module-level контракт для будущей замены Cust_SQLite.custom_request_c."""
...
def stage2_observe_request(bd: Any, sql: str) -> bool:
    """Неблокирующий hook. Без настроенного probe является no-op."""
