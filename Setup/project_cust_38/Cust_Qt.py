...
def migat(self, obj, i: int, j: int, n: int = 2):
    for _ in range(n):
...
def add_label_link(object_tbl, i,j, file, name, rgba:tuple,conn_func):
    lbl = QtWidgets.QLabel()
    # r,g,b,a = rgba
    # lbl.setStyleSheet(f'background-color:rgba{r,g,b,a};')
    ...
    lbl.linkActivated.connect(conn_func)
...
def add_image(item, i, j, path='', self = '',w = 16, h = 16,conn_func_click = None):
    ...
    if conn_func_click:
        lbl.clicked.connect(lambda: conn_func_click(path))
    ...
    def paint(self, painter: QtGui.QPainter, option, index: QtCore.QModelIndex):
        ...
        if self.colorful_edit:
            if col not in self.editable_col_nomera:
                ...
                rgb = (250, 250, 250)
            if self.load_links and isinstance(value, str) and (os.sep in value or '//' in value):
...
def fill_wtabl(dict_or_list, object, set_editeble_col_nomera={}, ogr_maxshir_kol=200,
               ...
               list_column_widths:list=[],StretchLastRow=False,tbl_vidget:tuple|None=None,count_unhide_rows=5, selectionBehavior="SelectItems",count_rows_cell_max=1, load_links=False):
    ...
    for i in range(start_fill, len(list_of_data)):
        for j in range(len(list_of_data[i])):
            ...
            # if colorful_edit:
            #     if not object_tbl.item(i-start_fill,j) == None:
            #         if j not in set_editeble_col_nomera:
            #             add_color_wtab_c(object_tbl, i-start_fill, j, 15, 15, 15)
            #             object_tbl.item(i-start_fill,j).setBackground(QtGui.QColor(240,240,240))
            #         else:
            #             add_color_wtab_c(object_tbl, i-start_fill, j, 5, 5, 5)
            #             object_tbl.item(i - start_fill, j).setBackground(QtGui.QColor(250, 250, 250))
            ...
            # =================================links/files==============
                if isinstance(text, str) and ( os.sep in text or '//' in text) :
                    ...
                    if len(files) == 1:
                        ...
                        clr = object_tbl.item(i - start_fill, j).background().color()
                        if F.is_link_dir(file):

                            add_label_link(object_tbl,i - start_fill,j,file,file,clr.getRgb(),lbl_linkActivated)
                        ...
                        if F.is_link_file(file):
                            ...
                            if F.keep_extention_c(file) in ('.jpg', '.jpeg', '.bmp', '.png'):
                                ...
                                if size_h_k > 240:
                                    size_h_k = 240
                                size_w_k = size_h_k*k
                                add_image(object_tbl,i - start_fill, j,file,"",F.round_up(size_w_k),F.round_up(size_h_k),lbl_linkActivated)
                                ...
                                clr = object_tbl.item(i - start_fill, j).background().color()
                                add_label_link(object_tbl, i - start_fill, j, file, name, clr.getRgb(), lbl_linkActivated)
...
def refill_tbl_into_msgbox_get_table(self,tbl:QtWidgets.QTableWidget,msg='Информация'):
    msgboxg_get_table(self,msg=msg,dict_or_list =tbl,btn0_name='ОК',disable_btn1=True,load_summ=True)
...
def load_icons(self:object,size:int=32):
    dir = os.path.join(self.base_path, 'icons')
...
class Dialog_tbl(QtWidgets.QDialog):  # диалоговое окно
    def __init__(self, parent, msg:str, dict_or_list, btn0_name:str="Ввод",
                 ...
                 func_btn0=None,selection_from_tbl=False,ExtendedSelection=True,selectRows=False,func_oform_filtr=None):
        ...
        if isinstance(dict_or_list, QtWidgets.QTableWidget):
            ...
            fill_wtabl(dict_or_list, tbl, height_row=25, auto_type=True, colorful_edit=colorful_edit)
        if func_oform_tbl:
            func_oform_tbl(tbl)
        if selectRows:
...
def msgboxg_get_table(self, msg, dict_or_list, btn0_name="Ввод", btn1_name="Отмена", func_validate=None,*args,
                      ...
                      selectRows=False,func_oform_filtr=None):
...
class TableValidator(QStyledItemDelegate):
    ...
    def __init__(self, fn_validator: Callable[[str, str], bool], parent: QtWidgets.QTableWidget) -> None:
        super().__init__(parent)
        self.parent = parent
    ...
    def on_change_combo(self, new_index, row, col, *args):#После выбора строки комбобока
        ...
        if head_item and isinstance(cell, QtWidgets.QComboBox) and prev != cell.currentText():
            ...
            if not self.fn_validator(header_text, cell.currentText()):
    ...
    def setModelData(self, editor, model, index):#После изменения содержимого ячейки
        ...
        if self.fn_validator(header, value):
