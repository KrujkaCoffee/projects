...
class _AttributeInfo:
    def __init__(self):
        ...
        self.alias: _AttributeInfoMeta = _AttributeInfoMeta(str, '', 'alias', 'для юзера', 'Имя', 'Имя атрибута', 5,
                                                            True, '🏷️')
        ...
        self.user_hidden: _AttributeInfoMeta = _AttributeInfoMeta(bool, False, 'user_hidden', 'для юзера аналог _',
                                                                  ...
                                                                  '👀')
        self.for_list: _AttributeInfoMeta = _AttributeInfoMeta(bool, True, 'for_list', 'для списка', 'Вывод в таблицах',
                                                               'Вывод в таблице списка', 20, True, '📋')
        self.for_details: _AttributeInfoMeta = _AttributeInfoMeta(bool, True, 'for_details', 'для деталировки',
                                                                  'Вывод в свойствах', 'Вывод в таблице свойства', 25,
                                                                  True, '🔍')
        self.for_new: _AttributeInfoMeta = _AttributeInfoMeta(bool, True, 'for_new', 'для создания',
                                                              'Вывод при создании', 'Вывод при создании нового объекта',
                                                              30, True, '✨')
        self.order: _AttributeInfoMeta = _AttributeInfoMeta(int, 99999, 'order', 'порядок отображения',
                                                            'Порядок вывода', 'Порядок вывода в таблицах', 35, True,
                                                            '🔢')
        self.for_report: _AttributeInfoMeta = _AttributeInfoMeta(bool, True, 'for_report', 'для отчета',
                                                                 'Вывод в отчетах', 'Вывод в отчетах', 40, True, '📊')
        self.report_user_hidden: _AttributeInfoMeta = _AttributeInfoMeta(bool, False, 'report_user_hidden',
                                                                         'для юзера аналог _', 'Скрытый в отчетах',
                                                                         'Скрытый в отчетах', 45, True, '👁️‍🗨️')
        self.attr_view: _AttributeInfoMeta = _AttributeInfoMeta(str, '', 'attr_view',
                                                                'одно или два поля представления составного типа',
                                                                'Представление типа',
                                                                'Какие поля формируют значение в ячейках', 3, True,
                                                                '🧩️')
        self.binding_spec: _AttributeInfoMeta = _AttributeInfoMeta(
            dict,
            None,
            'binding_spec',
            'Структурированное описание источника атрибута',
            'Подвязка источника',
            'Выбранные поля и маршрут связей',
            4,
            False,
            ''
        )
        ...
        self.master_attr: _AttributeInfoMeta = _AttributeInfoMeta(bool, False, 'master_attr', 'Мастер атрибут',
                                                                  'Мастер', 'Мастер атрибут для связей(уникальный)', 54,
                                                                  True, 12)
    ...
    def _template_new(self):
        data_attrs = [v for k, v in F.get_all_attrs(self).items() if
                      isinstance(v, _AttributeInfoMeta) and v.show_new_templ]
    ...
    def to_flat_dict(self) -> dict[str, object]:
        """
        Возвращает словарь {имя_поля: текущее_значение}
    ...
    @classmethod
    def from_flat_dict(cls, data: dict[str, object]) -> '_AttributeInfo':
        """
        Восстанавливает объект из плоского словаря.
        Предполагается, что словарь содержит только значения полей.
        """
...
class _Attribute(Generic[T]):
    ...
    def set_value(self, value: T, forced: bool = False):
        ...
        if value is None and isinstance(type_attr, type) and issubclass(type_attr, Mes_type):
            self.value = None
            return True
        if type_attr != type(value) and value is not None:
    ...
    def template_new(cls) -> tuple[list[dict], list[dict]]:
        ...
        for k, v in dict_text_attrs.items():
            tmp_dict_text = {'_name': k, '': dict_emoj[k], 'Свойство': dict_aliases[k], 'Значение': v,
                             'Описание': dict_descr[k]}
            tmp_dict_data = {'_name': k, '': dict_emoj[k], 'Свойство': dict_aliases[k], 'Значение': dict_data_attrs[k],
                             'Описание': dict_descr[k]}
    ...
    def attr(cls,
        ...
        obj_info.binding_spec = copy.deepcopy(binding_spec)
...
class AttributeEditDraft:

    LOCKED_FIELDS: set = frozenset(['type', 'base_attr'])

    def __init__(self, attribute: _Attribute):

        self.__original_info = copy.deepcopy(attribute.info)
        self.__working_info = copy.deepcopy(attribute.info)
        self.__before = copy.deepcopy(self.__original_info)

    @property
    def info(self):
        return self.__working_info
    ...
    @staticmethod
    def __snapshot(info: _AttributeInfo):
        result = {}

        for name, meta in object.__getattribute__(info, '__dict__').items():
            if not isinstance(meta, _AttributeInfoMeta):
                continue
            result[name] = copy.deepcopy(meta.val)
        return result

    def template(self) -> tuple[list[dict, list[dict]]]:
        """Возвращает шаблон редактора"""
        edit_info = copy.deepcopy(self.__working_info)
        (
            dict_text_attrs,
            dict_data_attrs,
            dict_aliases,
            ...
            dict_emoj,
            *args
        ) = edit_info._template_new()

        result_text = []
        result_data = []

        for name, value in dict_text_attrs.items():
            result_text.append({
                '_name': name,
                '': dict_emoj[name],
                'Свойство': dict_aliases[name],
                'Значение': value,
                'Описание': dict_descr[name]
            })
            result_data.append({
                '_name': name,
                '': dict_emoj[name],
                'Свойство': dict_aliases[name],
                'Значение': dict_data_attrs[name],
                'Описание': dict_descr[name]
            })
        return result_text, result_data

    def accept_values(self, values: dict[str, object]):
        """Принимает результат формы как черновик"""
        candidate = copy.deepcopy(self.__working_info)
        for name, value in values.items():
            before_dict = self.__before.to_flat_dict()
            before_value = getattr(self.__before, name)
            if name not in before_dict:
                raise KeyError(f'Передано неизвестное свойство: {name}')
            if (name in self.LOCKED_FIELDS
                and value != before_value):
                meta = candidate._meta(name)
                raise ValueError(f"Свойство {meta.alias_adduced} заблокировано для редактирования")
    ...
    def changes(self):
        ...
        for name in before_dict.keys():
            after = current[name]
            before = getattr(self.__before, name)

            if before == after:
                continue

            meta = self.__working_info._meta(name)
            result.append(
                AttributeInfoChange(
                    name=name,
                    caption=meta.alias_adduced,
...
class CustomTypes:
    ...
    def __init__(self, mes_types: PMES.PlannerMesTypeCatalog = None):
        self.TYPE_MAP = copy.deepcopy(self.TYPE_MAP)
        self.mes_types = mes_types
        self.mes_error = ''
    ...
    def refresh_mes_types(self):
        ...
        self.mes_error = ''
        self.mes_warnings = tuple()
        if self.mes_types is None:
            root.text = "Данные МЕС (не настроено)"
            return
        try:
            entries = self.mes_types.reload(Mes_type)
        ...
        self.mes_warnings = self.mes_types.warnings
    ...
    def deserialize_type(self, data) -> type:
        ...
        if kind == 'mes':
            ...
            return self.mes_types.type_for_source(
    ...
    def get_mainType_o_by_type(self, obj_or_value: object, drop_base: bool = False) -> MainTypes | str:
    ...
    def _add_types_tmplate(self, template, template_data, inner_data, parent: str = '', lvl=0) -> tuple[list, list]:
        ...
        for k, v in inner_data.items():
            ...
            tmp_dict_text = {'_name': name, "": v.emoji, '_parent': parent, '_selectable': v.selectable,
                             'Тип': f'{" " * 4 * lvl}{v.text}'}
            ...
            if v.inner_data:
                template, template_data = self._add_types_tmplate(template, template_data, v.inner_data,
                                                                  parent=f'{parent}{name}',
                                                                  lvl=lvl + 1)
        return template, template_data

    def validate_mes_binding(self, binding: AB.AttributeBinding) -> AB.BindingValidationResult:
        manager = AB.AttributeBindingManager()
        if not isinstance(binding, AB.AttributeBinding):
            raise TypeError('Передан некорректный аргумент binding')
        if any(field.provider != AB.SourceProvider.MES for field in binding.fields):
            raise ValueError('В переданном binding найдены некорректные fields')
        if self.mes_types is None:
            return manager.validate_againts(binding, None, 'Каталог справочников МЕС недоступен')
        source_key = binding.fields[0].source_key

        try:
            entries = self.mes_types.reload(Mes_type)
            self.mes_warnings = self.mes_types.warnings
            logging.debug(self.mes_warnings)
        except Exception as e:
            logging.error('Ошибка при запросе мес справочников')
            return manager.validate_againts(binding, None, "Не удалось запросить справочники МЕС")

        current_entry = next((item for item in entries if item.source_key == source_key), None)
        if current_entry is None:
            return manager.validate_againts(binding, ())

        try:
            choice = self.mes_types.choice_for_type(current_entry.value)

            current_fields = tuple(
                AB.BindingField(
                    provider=AB.SourceProvider.MES,
                    source_key=choice.source_key,
                    entity_key=presentation.result_table_key,
                    field_key=presentation.result_field_name,
                    presentation_key=presentation.presentation_key,
                    caption=presentation.caption,
                    relation_steps=tuple(presentation.relation_steps)
                )
                for presentation in choice.presentations
            )
        except Exception as e:
            return manager.validate_againts(binding, None, "Не удалось запросить справочники МЕС")

        return manager.validate_againts(binding, current_fields)

    def validate_erp_bindings(self, binding: AB.AttributeBinding
                              ) -> AB.BindingValidationResult:
        if not isinstance(binding, AB.AttributeBinding):
            raise TypeError('Передан некорректный аргумент binding')
        if any(field.provider != AB.SourceProvider.ERP for field in binding.fields):
            raise ValueError('В переданном binding найдены некорректные fields')
        source_key = binding.fields[0].source_key
        manager = AB.AttributeBindingManager()
        if source_key.startswith('ErpMetaClass.'): #todo
            type_path = source_key
        else:
        ...
        try:
            success, meta_fields = erp_type._get_fields()
        except Exception as e:
            return manager.validate_againts(binding, None,
                                            'Не удалось запросить метаданные ERP')
        if not success:
            return manager.validate_againts(
                binding,
                None,
                'ERP вернула ошибку при запросе справочника'
            )
        if not isinstance(meta_fields, list):
            return manager.validate_againts(
                binding,
                None,
                'ERP вернула некорректный формат справочника'
            )

        current_fields = []

        for item in meta_fields:
            field_name = item.get('Имя') or ''
            caption = item.get('Синоним') or field_name
            current_fields.append(
                AB.BindingField(
                    ...
                    presentation_key=field_name,
                    caption=caption,
                    relation_steps=(),
                )
            )
        return manager.validate_againts(binding, tuple(current_fields))
...
class Info():
    ALIASES = {'Дств': CEMOJ.ОперацииПроизводства.action.symbol}
    ...
    def __init__(self, ui_tbl, ui_btn_ok, ui_btn_cancel):
        ...
        self.dict_aliases: dict | None = None
    ...
    def __build_mes_binding(self, type_value, presentation_keys, origin, binding_manager: AB.AttributeBindingManager):
        ...
        presentations = choice.selected_presentations(presentation_keys)
        fields = [
            AB.BindingField(
                provider=AB.SourceProvider.MES,
                source_key=choice.source_key,
        ...
        return binding_manager.from_fields(
            fields,
            origin=origin
        )

    def __build_erp_binding(self, type_value, selected_rows,
                            origin: AB.BindingOrigin,
                            binding_manager: AB.AttributeBindingManager):
        source_key = DTSUB.custom_types.get_full_type_name(
            type_value,
            drop_base=True
        )
        fields = [
            AB.BindingField(
                ...
                source_key=source_key,
                entity_key=source_key,
                field_key=item['_name'],
                presentation_key=item['_name'],
                caption=item.get('Поле') or item['_name'],
                relation_steps=()
            )
    ...
    def _new_attr(self, edit_attribute: _Attribute | None = None) -> bool | None:
        ...
        def fnc_oform(tbl: CQT.QtWidgets.QTableWidget, *args):
            ...
            def fnc_cell_edit_bool(tbl: CQT.QtWidgets.QTableWidget, item: CQT.QtWidgets.QTableWidgetItem,
                                   t: CQT.TableContext):
                i = item.row()
                j = item.column()
                row = t.get_row(i)
                ...
                if attr_o_type is type:
                    ...
                    def fnc_select_type(lbl: CQT.InteractiveLabelInstance, sub_self, i, j, row: CQT.TableRow):
                        ...
                        rez = CQT.msgboxg_get_table(DTSUB.sub_self, 'Выбор типа данных', template,
                                                    ...
                                                    dict_or_list_user_data=template_data, selectRows=True,
                        ...
                        # ================================================================
                        # ====================name=================================
                        row_alias = row.ctx.find_row({'_name': 'alias'}, first=True)
                        row_alias.set_value('Значение', new_type.text)
                        row_alias.set_value('Значение', new_type.text, set_cust_content=True)
                        ...
                        if issubclass(new_meta.val, (Erp_type, Mes_type)):
                            ...
                            if issubclass(new_meta.val, Mes_type):
                                binding = self.__build_mes_binding(
                                    new_meta.val,
                                    None,
                                    AB.BindingOrigin.INFERRED,
                                    binding_manager
                                )
                                binding_state['value'] = binding

                                # presentation = DTSUB.mes_types.default_presentation(new_meta.val)
                                set_attr_view(row_view_type, binding.attr_view, binding.display_text)
                        else:
                        ...
                        def fnc_select_type_attr_view(lbl: CQT.InteractiveLabelInstance, sub_self, i, j,
                            ...
                            if issubclass(type, Erp_type):
                                ...
                                if not 1 <= len(selected) <= 2:
                                    return CQT.msgbox('Не выбрано не одного поля представления')

                                binding = self.__build_erp_binding(
                                    type,
                                    selected,
                                    AB.BindingOrigin.EXPLICIT,
                                    binding_manager
                                )
                                binding_state['value'] = binding


                                set_attr_view(row, binding.attr_view, binding.display_text)
                                ##

                                #
                                # new_type_attr_view = ';'.join([_['_name'] for _ in rez])
                                # new_type_attr_view_meta: _AttributeInfoMeta = row.value('Значение',
                                #                                                         get_cust_content=True)
        ...
        if DTSUB.current_settings_mode in (Type_entitys.Res, Type_entitys.Eve):
            ...
            if not isinstance(rez.get('type'), type):
                ...
                return
            if (
                    issubclass(rez['type'], (Mes_type, Erp_type))
                    and binding_state['value'] is None
            ):
            ...
            new_attr: _Attribute = _Attribute.attr(None, type_val=rez['type'], alias=rez['alias'],
                                                   ...
                                                   user_hidden=rez['user_hidden'], for_list=rez['for_list'],
                                                   for_details=rez['for_details'],
    ...
    def _fill_data(self):
        ...
        if self.dict_aliases:
            aliases.update(self.dict_aliases)
        CQT.fill_wtabl(self.transponate(self._raw_data, self.dict_aliases, self._dict_data), self.t.tbl,
                       ...
                       dict_or_list_user_data=self.transponate(self._dict_data, self.dict_aliases, self._dict_data),
    ...
    def transponate(template: dict, dict_aliases: dict = None, dict_data: dict = None) -> list[dict]:
        ...
        if dict_data is None:
            dict_data = dict()
        return [
            {'_name': k, '_data': dict_data.get(k, v), 'Свойство': dict_aliases.get(k, k), 'Дств': "", 'Значение': v}
...
class CustAttrs():
    ...
    def get_dict_attrs(self) -> dict[str, _Attribute]:
        return F.get_all_attrs(self, attr_type=_Attribute)

    def for_ui(self):
        data_cust_attr: dict[str, _Attribute] = F.get_all_attrs(self, attr_type=_Attribute)
        DTSUB.custom_types.refresh_mes_types()
        return [{'_name': k, 'ca_alias': v.info.alias,
                 'ca_type': DTSUB.custom_types.get_mainType_o_by_type(v.info.type).name if
                 isinstance(DTSUB.custom_types.get_mainType_o_by_type(v.info.type), MainTypes) else 'err',
                 'ca_del': '', 'ca_edit': '', 'ca_description': v.info.description} for k, v in data_cust_attr.items()]
    ...
    def aliases(cls):
        return {'ca_alias': 'Имя',
                ...
                'ca_del': '    ',
                'ca_edit': '    ',
                'ca_description': 'Описание'}

    def serialize(self) -> dict[dict]:
        dict_attrs: dict[str, _Attribute] = self.get_dict_attrs()
        return {k: v.serialize() for k, v in dict_attrs.items()}
    ...
    def add_new_attr(self, data):
        name = self._gen_new_cust_attr_name()
        data.info.base_attr = False
...
class _BaseEntity():
    ...
    def __init__(self, type_entity: Type_entity, id: int | None = None, name: str = '', description: str = '',
                 emoj: str = CEMOJ.СтатусыПроизводства.selected.symbol, for_delete: bool = False,
                 color: Color | None = None):
        if color is None:
            color = Color.random()
        self.id: _Attribute[int] = _Attribute.attr(id, int, alias='№', description='', protected=True, user_hidden=True,
                                                   for_list=True, for_details=True, for_new=True,
                                                   report_user_hidden=True, order=1)
        self.type_entity: _Attribute[Type_entity] = _Attribute.attr(type_entity, Type_entity, alias='Тип',
                                                                    description='', protected=True, user_hidden=True,
                                                                    for_list=True, for_details=True, order=66)
        self.emoj: _Attribute[str] = _Attribute.attr(emoj, str, alias='', description='', protected=False,
                                                     user_hidden=False, for_list=True, for_details=True, order=23)
        self.name: _Attribute[str] = _Attribute.attr(name, str, alias='Имя', description='', protected=False,
                                                     user_hidden=False, for_list=True, for_details=True, order=44)
        self.description: _Attribute[str] = _Attribute.attr(description, str, alias='Описание', description='',
                                                            protected=False, user_hidden=False, for_list=True,
                                                            for_details=True, order=55)
        self.for_delete: _Attribute[bool] = _Attribute.attr(for_delete, bool, alias='Удал.', description='',
                                                            protected=False, user_hidden=False, for_list=True,
                                                            for_details=True, for_report=False, order=12)
    ...
    def upadte_child_attrs(self, resources: 'Resources'):
        for it in resources.get_by_shablon(self.id.value):
            it: 'Resource'
            attrs_sh = self.cust_attrs.value.get_dict_attrs()
            for name, cust_attr in attrs_sh.items():
                if hasattr(it, name):
                    continue
                setattr(it, name, copy.deepcopy(cust_attr))
    ...
    def to_ui(self, attr_name) -> str:
        ...
        if attr_name == 'cust_attrs':
            attr_in: CustAttrs = attr_val.value
            return attr_in.for_ui()
...
class _BaseDimension:
    ...
    def __init__(self, id_shablon: int, id: int, name: str = '', descr: str = ''):
        self.id: _Attribute[int] = _Attribute.attr(id, int, alias='№', description='', protected=True, user_hidden=True,
                                                   for_list=True, for_details=True, for_new=False,
                                                   report_user_hidden=True, order=0)
        self.name: _Attribute[str] = _Attribute.attr(name, str, alias='Название', description='', protected=False,
                                                     user_hidden=False, for_list=True, for_details=True, for_new=True,
                                                     order=3)
        self.descr: _Attribute[str] = _Attribute.attr(descr, str, alias='Описание', description='', protected=False,
                                                      user_hidden=False, for_list=True, for_details=True, for_new=True,
                                                      order=9)
        self.shablon: _Attribute[int] = _Attribute.attr(id_shablon, int, alias='Шаблон', description='', protected=True,
                                                        user_hidden=False, for_list=True, for_details=True,
                                                        for_new=True, order=8)
        self.emoj: _Attribute[str] = _Attribute.attr('', str, alias='', description='', protected=False,
                                                     user_hidden=False, for_list=True, for_details=True, for_new=False,
                                                     order=2)
        self.for_delete: _Attribute[bool] = _Attribute.attr(False, bool, alias='❌', description='На удаление',
                                                            protected=False, user_hidden=False, for_list=True,
                                                            for_details=True, for_new=False, for_report=False, order=1)
        self.color: _Attribute[Color] = _Attribute.attr(Color.random(), Color, alias='Цвет', description='',
                                                        protected=False, user_hidden=False, for_list=False,
                                                        for_details=True, for_new=False, report_user_hidden=True,
                                                        order=3)
    ...
    def get_dict_cust_attrs(self) -> dict[str, _Attribute]:
        return {k: v for k, v in F.get_all_attrs(self, attr_type=_Attribute).items() if not v.info.base_attr}
    ...
    def template_new(self) -> tuple[list[dict], list[dict]]:
        attrs: list[tuple[str, _Attribute]] = [(k, _) for k, _ in F.get_all_attrs(self).items() if
                                               isinstance(_, _Attribute)]
        text = [{'_name': t[0], '_data': str(self.to_service_val(t[0])), 'Параметр': t[1].info.alias,
                 'Значение': self.to_ui(t[0])} for t in attrs if t[1].info.for_new]
        data = [{'_name': t[0], '_data': str(self.to_service_val(t[0])), 'Параметр': t[1].info.alias,
                 'Значение': self.to_service_val(t[0])} for t in attrs if t[1].info.for_new]
        return text, data
...
class Event(_BaseDimension):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.start: _Attribute[Cdt] = _Attribute.attr(Cdt(), Cdt, alias='Начало', description='Начало события',
                                                      protected=False,
                                                      user_hidden=False, for_list=True, for_details=True, for_new=False,
                                                      order=22)
        self.end: _Attribute[Cdt] = _Attribute.attr(Cdt(), Cdt, alias='Конец', description='Конец события',
                                                    protected=False,
                                                    user_hidden=False, for_list=True, for_details=True, for_new=False,
                                                    order=23)
...
class _BaseDimensions:
    ...
    def get_by_shablon(self, id_shablon: int) -> list[_TYPE_CLS]:
        return [_ for _ in self.dict_elems.values() if _.shablon.value == id_shablon]
    ...
    def template_list(self, include_deleted: bool = True) -> tuple[list[dict], list[dict], dict]:
        ...
        to_service_list = []
        if not accessed_shablons:
            return to_ui_list, to_service_list, full_aliases

        for _ in accessed_shablons:
            to_ui, to_service, aliases = _.template()
            full_aliases.update(aliases)
            to_ui_list.append(to_ui)
            to_service_list.append(to_service)
...
class Cross():
    def __init__(self, id_res, id_eve):
        self.id: _Attribute[int] = _Attribute.attr(-1, int, alias='№', description='', protected=True, user_hidden=True,
                                                   for_list=True, for_details=True, for_new=False,
                                                   report_user_hidden=True, order=0)
        self.res: _Attribute[int] = _Attribute.attr(id_res, int, alias='Ресурс', description='', protected=True,
                                                    user_hidden=False, for_list=True, for_details=True, for_new=True,
                                                    report_user_hidden=True, order=1)
        self.eve: _Attribute[int] = _Attribute.attr(id_eve, int, alias='Событие', description='', protected=True,
                                                    user_hidden=False, for_list=True, for_details=True, for_new=True,
                                                    report_user_hidden=True, order=3)
        self.start_eve: _Attribute[None] = _Attribute.attr(None, None, alias='Начало соб.',
                                                           description='Начало события', protected=True,
                                                           user_hidden=False, for_list=True, for_details=False,
                                                           for_new=False, for_report=False,
                                                           order=22)
        self.end_eve: _Attribute[None] = _Attribute.attr(None, None, alias='Конец соб.', description='Конец события',
                                                         protected=True,
                                                         user_hidden=False, for_list=True, for_details=False,
                                                         for_new=False, for_report=False,
                                                         order=23)
        self.start: _Attribute[Cdt] = _Attribute.attr(Cdt(), Cdt, alias='С', description='Начало Участия',
                                                      protected=False,
                                                      user_hidden=False, for_list=True, for_details=True, for_new=False,
                                                      order=25)
        self.end: _Attribute[Cdt] = _Attribute.attr(Cdt(), Cdt, alias='По', description='Конец Участия',
                                                    protected=False,
                                                    user_hidden=False, for_list=True, for_details=True, for_new=False,
                                                    order=26)
...
class Crosses():
    ...
    def calc_dates(
            self):  #TODO расчет дат с приоритетом события( если не указана дата события, то берется крайние даты Участий)
        pass
...
class CrossEntity():
    def __init__(self):
        self.res: _Attribute[Resource] = _Attribute.attr(None, Resource, alias='Ресурс', description='',
                                                         protected=False,
                                                         user_hidden=False, for_list=True, for_details=True,
                                                         for_new=True,
                                                         order=1)
        self.eve: _Attribute[Event] = _Attribute.attr(None, Event, alias='Событие', description='', protected=False,
                                                      user_hidden=False, for_list=True, for_details=True, for_new=True,
                                                      order=5)
        self.res_sh: _Attribute[ShablonRes] = _Attribute.attr(None, ShablonRes, alias='ШаблонРес', description='',
                                                              protected=False,
                                                              user_hidden=False, for_list=True, for_details=True,
                                                              for_new=True,
                                                              order=10)
        self.eve_sh: _Attribute[ShablonEve] = _Attribute.attr(None, ShablonEve, alias='ШаблонСоб', description='',
                                                              protected=False,
                                                              user_hidden=False, for_list=True, for_details=True,
                                                              for_new=True,
                                                              order=15)
    ...
    def template(self) -> tuple[dict, dict, dict, dict]:
        ...
        list_cross_entity_attrs = [(name, _) for name, _ in F.get_all_attrs(self).items() if
                                   isinstance(_, _Attribute) and _.info.for_report]
        ...
        for cross_entity_attr_name, cross_entity_attr_o in list_cross_entity_attrs:
            ...
            list_data_fields = [(name, _) for name, _ in F.get_all_attrs(cross_entity_attr_o.value).items() if
                                isinstance(_, _Attribute) and _.info.for_report]
...
class CrossManager():
    ...
    @staticmethod
    def templates_pivot(list_crosses: list[CrossEntity], order_res: dict[int, int] = None) -> tuple[
        list[dict], list[dict], list[dict], list[dict], dict, dict]:
        if not list_crosses:
            return ([], [], [], [], {}, {})
        list_text_v_sub = []
        list_data_v_sub = []
        ...
        [[dict_descr.update({k: v.info.description}) for k, v in F.get_all_attrs(_.value, attr_type=_Attribute).items()]
         for _ in list_res]
