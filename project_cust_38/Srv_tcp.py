...
class HTTPSrv:
    ...
    def try_serve_from_cache(self, *, request_key: str, request_headers: dict[str, str]):
        ...
        try:
            ...
            logger.error(f'[try_serve_from_cache] payload deserialize error: {e}')
            traceback.print_exc()
            return entry, False  # fallback — идём в БД
    ...
    def execute_sql(self, *, bd, custom_request_c, hat_c, list_of_lists_c, rez_dict, one, one_column, attach_dbs):
        ...
        if conn is False:
            logger.error(f'[execute_sql] не удалось подключиться к {bd}')
            return None
        try:
            ...
            logger.error(f'[execute_sql] PRAGMA error: {e}')
