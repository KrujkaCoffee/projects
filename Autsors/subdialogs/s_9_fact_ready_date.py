...
class FactReadyDate(QtWidgets.QDialog):
    def __init__(self, application_num):
        super().__init__()
        self.application_num = application_num
 
        self.setWindowTitle(f"Внесение фактической даты готовности {self.application_num}")
        self.resize(227, 100)
        verticalLayout = QtWidgets.QVBoxLayout()

        self.need_date_label = QtWidgets.QLabel('фактическая дата')
    ...
    def save_application(self):
        ...
        di['fact_ready_date'] = check_input_values(self.need_date, self.need_date_label)
        
        if di['fact_ready_date']:
            result = refact.update_fact_ready_date(self.application_num, check_input_values(self.need_date, self.need_date_label))
            if result:
                CQT.msgbox('Удачно')
            else:
                CQT.msgbox('Не удалось изменить дату')
            # res = post_request('/outsouce/set_fact_ready_date', di)
            self.close()