...
@CQT.onerror
def obn_mat_erp_file(self, *args):
    def load_from_file(self, file_erp, path_dir):
        keys_dict={'ЕдиницаИзм':'ЕдиницаИзмерения'}
        file_erp = F.deploy_dict_c(file_erp, 'Код')
        list_nomen_db = CSQ.custom_request_c(self.db_mater,f"""SELECT * FROM nomen""",rez_dict=True)
        ...
        for field in dict_change['change'].keys():
            ...
            for item in dict_change['change'][field]:
                ...
                if counter>= limit_counter:
                    CSQ.custom_request_c(self.db_mater,f"""UPDATE nomen SET ({field}, Дата_изменения) =
        ...
        for item in dict_change['del']:
            ...
            counter += 1
            if counter >= limit_counter:
                CSQ.custom_request_c(self.db_mater, f"""UPDATE nomen SET (На_удаление, Дата_изменения) =
                                         (?, ?) WHERE Код = ?;""", list_of_lists_c=strok_input)
                strok_input = []
                counter = 0
                F.sleep(1)
