...
RETRY_CALL_CRC_KEY = 'RETRY_CALL_CRC_KEY'
...
def upload_resource(body: ResourceRequest):
    try:
        ...
        last_try = os.environ.get(RETRY_CALL_CRC_KEY)
        now = F.now('')
        if not last_try or (now - datetime.strptime(last_try, '%Y-%m-%d %H:%M:%S')).total_seconds() > 60 * 60:
            os.environ[RETRY_CALL_CRC_KEY] = F.now()
            import importlib
            importlib.reload(CRC)
            return upload_resource(body)
        raise e
    return spec.send()
